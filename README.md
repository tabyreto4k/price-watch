...
Публичной ссылки на бота здесь нет. Своего поднимаешь [тремя командами](#запуск-локально),
...
Готовый образ каждой ревизии `main` лежит в GHCR. Тот же стек поднимается из него, ничего
не собирая локально:
...
Проверка (порт 8080 публикует только dev-компоуз, в проде его нет):
...
доказывает. Выкатывается не `latest`, а конкретный `sha-<полный sha>`, так что в логе
прогона видно, какая именно ревизия уехала. `docker compose up --wait` делает healthcheck частью выката:
...
- Rate limiter на источник (Resilience4j): Wildberries не чаще 1 rps, HTML-страницы 0.5 rps.
- Ретраи с экспоненциальным backoff и jitter — только на 5xx и таймауты. Повтор 4xx вернул
  бы тот же ответ, поэтому его нет.
- Таймауты: 3 секунды на соединение, 5 на чтение.
- User-Agent с ссылкой на репозиторий: видно, кто пришёл и куда писать.
- Точка истории пишется только при изменении цены. Проверка раз в 30 минут иначе дала бы
  ~17 тыс. одинаковых строк на товар в год.
- HTML-источник работает по allow-list доменов, на каждый прописан селектор.
...
Три решения расписаны подробно в [`docs/adr/`](docs/adr), вместе с тем, чем за них платим.
